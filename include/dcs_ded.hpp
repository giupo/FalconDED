...
DcsBios::StringBuffer<LINE_LENGTH> dedLine2Buffer(0x451a, onDedLine2Change);

void onDedLine3Change(char* newValue) {  
  strncpy(DED[2], newValue, LINE_LENGTH);
}
DcsBios::StringBuffer<LINE_LENGTH> dedLine3Buffer(0x4534, onDedLine3Change);

void onDedLine4Change(char* newValue) {    
  strncpy(DED[3], newValue, LINE_LENGTH);
}
DcsBios::StringBuffer<LINE_LENGTH> dedLine4Buffer(0x454e, onDedLine4Change);

void onDedLine5Change(char* newValue) {  
  strncpy(DED[4], newValue, LINE_LENGTH);
}
DcsBios::StringBuffer<LINE_LENGTH> dedLine5Buffer(0x4568, onDedLine5Change);


void dcs_setup() {
	strncpy(DED[4], "Waiting DCS...", LINE_LENGTH);
}
